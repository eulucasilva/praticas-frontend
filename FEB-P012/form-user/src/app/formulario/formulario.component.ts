...
export class FormularioComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.formChangesSubscription = this.fs.formChanges$.subscribe((formChanges) => {
      console.log('Formulário alterado:', formChanges);
    });

    this.formUsuario.valueChanges.subscribe(() => {
      this.fs.updateFormChanges(this.formUsuario);
    });
  ...
  submitForm() {
    if (this.formUsuario.valid) {
      const formData = this.fs.getFormData();;
      console.log('Formulário submetido:', formData);
    }

    this.fs.printFormData();
  }
